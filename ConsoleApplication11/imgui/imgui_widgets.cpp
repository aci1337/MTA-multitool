...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (pressed)
        MarkItemEdited(id);
    ImColor ButtonColor = ImColor(0.106f, 0.149f, 0.165f, 1.000f);
    // Render
...
ImColor lineCol = { 135, 206, 250, 160 };
// 0, 0, 0, 255
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.y + 0.01 * 2, label_size.y + 0.01 * 2));
    ImGui::ItemSize(check_bb, 0.01);

    ImRect total_bb = check_bb;
    if (label_size.x > 0)
        ImGui::SameLine(0, style.ItemInnerSpacing.x);
    const ImRect text_bb(window->DC.CursorPos + ImVec2(0, 0.01), window->DC.CursorPos + ImVec2(0, 0.01) + label_size);
    if (label_size.x > 0)
    {
        ImGui::ItemSize(ImVec2(text_bb.GetWidth(), check_bb.GetHeight()), 0.01);
    ...
    ImGui::RenderFrame(check_bb.Min, check_bb.Max, ImGui::GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, 0);
    if (*v)
    {
        const float check_sz = ImMin(check_bb.GetWidth() - 2, check_bb.GetHeight());
        const float pad = ImMax(1.0f, (float)(int)(check_sz / 4.0f));
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad), check_bb.Max - ImVec2(pad, pad), ImGui::GetColorU32(ImGuiCol_CheckMark), 0);
    }
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, float power, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    if (axis == ImGuiAxis_X)
        ...
        *out_grab_bb = ImRect(bb.Min.x, grab_pos - grab_sz * 0.5f, bb.Max.x - grab_padding, grab_pos + grab_sz * 0.5f);
...
//    if (value_changed)
//        MarkItemEdited(id);
//    const ImU32 col_grab = IM_COL32(255, 255, 255, 255);
//    // Render grab
...
//        window->DrawList->AddCircleFilled(ImVec2((grab_bb.Min.x + grab_bb.Max.x) / 2, grab_bb.GetCenter().y), 4, ImColor(0, 162, 232, 255));
//    }
//    /*window->DrawList->AddRectFilledMultiColor(ImVec2(frame_bb.Min.x + 1, frame_bb.Min.y + 3.25), ImVec2(grab_bb.Max.x - 3, grab_bb.Max.y), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.38f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.38f)));*/
//    //window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
//    //window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + 6, grab_bb.Min.y + ((grab_bb.Max.y - grab_bb.Min.y) / 2)), 13, col_grab, (g.ActiveId == id ? ImVec4(0.76f, 0.10f, 0.24f, 1.00f)/*Active*/ : ImVec4(0.76f, 0.10f, 0.24f, 1.00f), 0.2f));
//
//
//    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
//    char value_buf[64];
...
//
//    //outlines
//    //window->DrawList->AddText(ImVec2(frame_bb.Min.x + 4, frame_bb.Min.y + 4), GetColorU32({ 0.2f, 0.2f, 0.2f, 0.8f }), value_buf);
//    // window->DrawList->AddText(ImVec2(frame_bb.Min.x + 2, frame_bb.Min.y + 2), GetColorU32({ 0.4f, 0.4f, 0.4f, 0.8f }), value_buf);
//    // window->DrawList->AddText(ImVec2(frame_bb.Min.x + 2, frame_bb.Min.y + 3), GetColorU32({ 0.4f, 0.4f, 0.4f, 0.8f }), value_buf);
//    // window->DrawList->AddText(ImVec2(frame_bb.Min.x + 3, frame_bb.Min.y + 2), GetColorU32({ 0.4f, 0.4f, 0.4f, 0.8f }), value_buf);
//
//     //actual text
//    window->DrawList->AddText(ImVec2(frame_bb.Max.x - 15, frame_bb.Min.y - 11), GetColorU32(ImGuiCol_Text), value_buf);
//
//
//
//    // RenderTextClipped(ImVec2(frame_bb.Min.x + 1, frame_bb.Min.y + 1), ImVec2(frame_bb.Max.x + 1, frame_bb.Max.y + 1), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
//   // RenderTextClipped(ImVec2(frame_bb.Min.x - 1, frame_bb.Min.y - 1), ImVec2(frame_bb.Max.x - 1, frame_bb.Max.y - 1), frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
//   // RenderTextClipped(ImVec2(frame_bb.Min.x - 1, frame_bb.Min.y + 1), ImVec2(frame_bb.Max.x + 1, frame_bb.Max.y + 1), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
//   // RenderTextClipped(ImVec2(frame_bb.Min.x + 1, frame_bb.Min.y - 1), ImVec2(frame_bb.Max.x + 1, frame_bb.Max.y + 1), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
//    if (label_size.x > 0.0f)
//        RenderText(ImVec2(frame_bb.Min.x - 12 + style.ItemInnerSpacing.x, frame_bb.Min.y - 16 + style.FramePadding.y), label);
//
//    return value_changed;
//}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 1, frame_bb.GetCenter().y - 1), ImVec2(frame_bb.Max.x - 10, frame_bb.GetCenter().y + 1), ImColor(1.0f, 1.0f, 1.0f, 4.000f), style.GrabRounding);
    //window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 1, frame_bb.GetCenter().y - 1), ImVec2(grab_bb.Max.x - 3, grab_bb.GetCenter().y), ImColor(0, 162, 232, 455), style.GrabRounding);
    //window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 1, frame_bb.GetCenter().y - 1), ImVec2(grab_bb.Max.x - 3, grab_bb.GetCenter().y), ImColor(0, 162, 232, 455), style.GrabRounding);
    //window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 1, frame_bb.GetCenter().y - 1), ImVec2(grab_bb.Max.x - 3, grab_bb.GetCenter().y), ImColor(0, 162, 232, 455), style.GrabRounding);
    //window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x + 1, frame_bb.GetCenter().y - 1), ImVec2(grab_bb.Max.x - 3, grab_bb.GetCenter().y), ImColor(0, 162, 232, 455), style.GrabRounding);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddCircleFilled(ImVec2((grab_bb.Min.x + grab_bb.Max.x) / 2, grab_bb.GetCenter().y), 8, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab));
        window->DrawList->AddCircleFilled(ImVec2((grab_bb.Min.x + grab_bb.Max.x) / 2, grab_bb.GetCenter().y), 4, ImColor(255, 255, 255, 255));
    }
    ...
    //outlines
    window->DrawList->AddText(ImVec2(frame_bb.Max.x - 15, frame_bb.Min.y - 11), GetColorU32({ 0.2f, 0.2f, 0.2f, 0.8f }), value_buf);
    // window->DrawList->AddText(ImVec2(frame_bb.Min.x + 2, frame_bb.Min.y + 2), GetColorU32({ 0.4f, 0.4f, 0.4f, 0.8f }), value_buf);
    // window->DrawList->AddText(ImVec2(frame_bb.Min.x + 2, frame_bb.Min.y + 3), GetColorU32({ 0.4f, 0.4f, 0.4f, 0.8f }), value_buf);
    // window->DrawList->AddText(ImVec2(frame_bb.Min.x + 3, frame_bb.Min.y + 2), GetColorU32({ 0.4f, 0.4f, 0.4f, 0.8f }), value_buf);
